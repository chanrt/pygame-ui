...
class Button:
    def __init__(self, x, y, width, height, screen, text):
        ...
        self._font_style = "arial"
        self._font_size = height // 2
    ...
    def set_font(self, font):
        self._font = font
        self.display_text = self.font.render(self._text, True, self._text_color)
        self.text_rect = self.display_text.get_rect(center=(self.x + self.width / 2, self.y + self.height / 2))
    ...
    def init_display_text(self):
        self._font = pg.font.SysFont(self._font_style, self._font_size)
    ...
    def render(self):
        if self.display:
            ...
            pg.draw.rect(self.screen, color, (self.x, self.y, self.width, self.height))
            if self.border:
                pg.draw.rect(self.screen, self.border_color, (self.x, self.y, self.width, self.height), self.border_width)
